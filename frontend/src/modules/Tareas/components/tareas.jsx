...
const Tareas = () => {
  ...
  const columns_forms = ['titulo', 'descripcion', 'nota', 'asignado_a', 'estado']
  const fetchData = (page) => {
    api.get(`/tarea/list/?page_size=${page}`)
      .then((res) => {
  ...
  const handleCreate = (newUser) => {
    api.post("/tarea/crear/", newUser).then(() => {
      fetchData(currentPage);
    });
  };
  ...
  const handleUpdate = (id, updatedUser) => {
    console.log(id)
    api.patch(`/tarea/modificar/${id}/`, updatedUser) // Asegúrate de que la URL esté bien construida
